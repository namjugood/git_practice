...
`gitsim`은 실제 `git` 바이너리와, 학습자가 직접 만들어 등록한 **진짜 원격 저장소**(예: GitHub)를
이용해 merge conflict, 브랜치 삭제, 강제 푸시 사고 같은 실무에서 자주 벌어지는 상황을 **직접
...
(`REPORT.md`)를 만들어줍니다. 성공한 시도는 등록한 원격 저장소에도 `README.md`로
자동 push되므로, 연습한 이력이 그대로 쌓여 포트폴리오처럼 볼 수 있습니다.
...
3. **연습 이력이 진짜 원격 저장소에 그대로 남아야 한다** → 강제 push, 브랜치 삭제 같은
   "사고" 시나리오도 실제 원격을 대상으로 연습하되, 워크스페이스마다 고유한
   `practice/<워크스페이스이름>` 브랜치에서만 동작하므로 실수를 포함한 모든 시도 기록이
   덮어써지지 않고 남습니다.
...
- **직접 채워야 하는 단계** (이름/이메일, 커밋 메시지, 브랜치 이름 등): 일부러
  ...
  통과됩니다 — 예시 문구를 그대로 실행하면 `gitsim learn check` 가 실패합니다.

`gitsim learn check` 가 실패하면 "왜" 실패했는지 진단(`[진단] ...`)을 함께
보여줍니다. 예를 들어 3번 단계에서 만든 브랜치와 다른 브랜치에서 커밋했다면
그 사실을 그대로 짚어줍니다 — 막연히 "확인해보세요" 로 끝나지 않습니다.
...
"원격(origin) 연결 및 push" 단계는 아래 [2) 실무 시나리오를 위한 원격 저장소
등록](#2-실무-시나리오를-위한-원격-저장소-등록)에서 설명하는 `gitsim remote set`
을 먼저 실행해서 실제 원격 저장소를 등록해두어야 진행할 수 있습니다. 아직
등록하지 않았다면 튜토리얼이 그 단계에서 등록 방법을 다시 안내해줍니다.

### 2) 실무 시나리오를 위한 원격 저장소 등록

시나리오 훈련은 로컬에서 흉내낸 가짜 원격이 아니라, 학습자가 직접 만든 **진짜 원격
저장소**(GitHub 등)를 대상으로 진행합니다. 먼저 GitHub 등에서 연습 전용 빈 저장소를
하나 만든 뒤, 그 주소를 gitsim에 등록하세요.

```bash
gitsim remote set https://github.com/<내계정>/<연습용저장소>.git
```

등록한 주소는 `~/.gitsim/config.json` 에 저장되어 이후 모든 시나리오가 계속
사용합니다. `gitsim remote show` 로 현재 등록된 주소를 언제든 확인할 수 있습니다.

> gitsim은 이 저장소에서 **`practice/*` 로 시작하는 브랜치만** 만들고 사용합니다.
> 기존에 있던 다른 브랜치(`main`, `master` 등)는 절대 건드리지 않지만, 그래도 실제로
> 협업 중인 저장소가 아니라 연습 전용으로 새로 만든 저장소를 등록하는 것을 권장합니다.
> 강제 push, 브랜치 삭제 같은 "사고" 시나리오도 이 저장소를 대상으로 안전하게
> 연습할 수 있도록 워크스페이스마다 고유한 브랜치를 사용하기 때문에, 어떤 시도도
> 다른 시도의 기록을 덮어쓰지 않습니다.
...
# ... 실제 git 명령으로 문제 해결 ...
gitsim check                      # 성공 여부 판정 + REPORT.md 생성 (+ 성공 시 원격에 README.md push)
...
성공으로 판정되면, 그 결과 리포트가 `README.md` 라는 이름으로 등록된 원격 저장소의
`practice/<워크스페이스이름>` 브랜치에 자동으로 push됩니다. GitHub에서 그 브랜치를
열어보면 이번 시나리오의 실제 git 히스토리와 함께 리포트가 바로 보이므로, 여러 번
연습한 기록을 포트폴리오처럼 공유할 수 있습니다. (실패한 시도는 원격에 push하지
않습니다 — 아직 로컬에서 계속 고쳐나가는 중일 수 있기 때문입니다.)
...
`repo/` 의 `origin` 은 `gitsim remote set` 으로 등록한 실제 원격 저장소(예: GitHub)를
가리킵니다. 워크스페이스마다 `practice/<워크스페이스이름>` 이라는 고유한 브랜치에서만
동작하므로, 다른 시나리오/다른 시도가 같은 원격 저장소를 공유해도 서로의 기록을
덮어쓰지 않습니다. (`remote.git/` 은 자동화 테스트에서만 로컬 bare 저장소를 가짜
원격으로 꽂아 넣을 때 쓰이며, 실제 실행에서는 만들어지지 않습니다.)
...
성공으로 판정된 경우, 이 내용이 `README.md` 라는 이름으로 그대로 등록된 원격
저장소의 `practice/<워크스페이스이름>` 브랜치에도 push됩니다.
...
화면에 보이는 모든 문구(브리핑, 진단, 모범 답안, CLI 메시지 등)는 코드에 직접
박혀 있지 않고 `gitsim/messages/ko/` 아래 파이썬 모듈들의 `MESSAGES: dict[str, str]`
에 키-값으로 정리되어 있습니다. 로직 코드(`gitsim/*.py`, `gitsim/scenarios/*.py`)는
`from gitsim.i18n import t as _` 로 가져온 뒤 `_("어떤.키", 값=변수)` 형태로 문구를
조회만 합니다.

새 언어를 추가하려면:

1. `gitsim/messages/ko/` 폴더 구조를 그대로 복사해서 `gitsim/messages/<로케일>/`
   (예: `en`)을 만듭니다.
2. 각 모듈의 `MESSAGES` 딕셔너리 **값만** 번역합니다. 키 이름은 코드가 참조하므로
   절대 바꾸면 안 됩니다.
3. `gitsim/messages/<로케일>/__init__.py` 도 `ko/__init__.py` 와 동일한 구조로
   하위 모듈들의 `MESSAGES` 를 모아서 노출해야 합니다.
4. `GITSIM_LOCALE=<로케일>` 환경 변수로 실행하면 그 언어로 보입니다. 특정 키가
   새 로케일에 아직 번역되어 있지 않으면 자동으로 한국어(ko)로 대체되므로,
   번역을 한 번에 다 끝내지 않고 점진적으로 채워나갈 수 있습니다.
...
gitsim/messages/
└── ko/
    ├── common.py       # 여러 곳에서 쓰는 짧은 라벨/공통 에러 메시지
    ├── cli.py          # `gitsim` 커맨드 자체의 출력 문구
    ├── tutorial.py     # `gitsim learn` 튜토리얼의 모든 문구
    ├── report.py       # REPORT.md 의 고정 틀(제목/구획 이름)
    └── scenarios/      # 시나리오별 브리핑/진단/모범 답안 (파일당 시나리오 1개)
```
...
실행하면 성공으로 판정되는지를 자동으로 검증합니다. 튜토리얼도 전체 8단계를 순서대로
수행하는 통합 테스트가 있습니다.
